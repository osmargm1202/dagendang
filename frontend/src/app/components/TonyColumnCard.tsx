    ...
    <section className="bg-card border border-border p-6 relative overflow-hidden">
      ...
      <div className="flex items-center gap-4 mb-4 relative z-10">
        {opinion?.authorPhoto ? (
          ...
          <div className="w-16 h-16 rounded-full bg-surface-container border border-border flex items-center justify-center font-serif font-bold text-xs text-foreground">TDR</div>
        )}
        <div>
          <h3 className="font-serif text-xl font-bold text-dr-blue dark:text-white">{opinion?.authorName || "Tony D. Reyes"}</h3>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">{opinion?.authorPosition || "Columnista"}</p>
        </div>
      </div>
      <h4 className="font-serif text-2xl font-semibold italic text-dr-blue dark:text-white mb-3 relative z-10">
        {opinion?.title || "La mirada del día"}
      </h4>
      <p className="text-foreground leading-relaxed mb-4 relative z-10 line-clamp-4">
