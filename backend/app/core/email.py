...
SMTP_FROM = os.getenv("SMTP_FROM", "La Agenda <noreply@delioserver.duckdns.org>")
...
async def send_verification_email(email: str, token: str):
    ...
    message["To"] = email
    message["Subject"] = "Verifica tu cuenta en La Agenda"
    
    # Updated link to the actual production/test domain
    verification_link = f"https://diariodigital.delioserver.duckdns.org/verificar?token={token}"
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 12px;">
        <h2 style="color: #0d47a1;">¡Bienvenido a La Agenda!</h2>
    ...
    """
    message.set_content(f"Hola, verifica tu cuenta en La Agenda usando este enlace: {verification_link}")
