...
deploy host="osmarg@10.0.0.13" port=port:
    ...
    ssh {{host}} 'docker run -d --name {{container}} --restart unless-stopped --network {{network}} --ip {{web_ip}} -p 127.0.0.1:{{port}}:80 -e PORT=80 --env-file ~/Code/dagendang-web.env {{image}}'
